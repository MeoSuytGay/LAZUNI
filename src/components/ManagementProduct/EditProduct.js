...
export const EditProduct = ({ existingProduct, onSave }) => {
    ...
    const [category, setCategory] = useState(existingProduct.category || 'fashion');
    const [condition, setCondition] = useState(existingProduct.condition || 'new');
    const [transactionType, setTransactionType] = useState(existingProduct.transactionType || 'sale'); 
    ...
    useEffect(() => {
        ...
        if (existingProduct.images) {
            setPreviewUrls(existingProduct.images.map(image => image.url)); // Adjust this depending on your image structure
        }
    }, [existingProduct]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            onSave(); // Call the onSave prop to refresh or redirect after saving
        } catch (error) {
